...
The data model is an organisation with many legal entities, and tenancy moves from entity to organisation with an entity ID on every ledger row, account, document and policy. Each entity carries its own KYB case, its own accounts and its own books, while the organisation carries the verified owners, control person and risk rating that a new entity inherits, which is exactly why the second application is short. Adding an entity is the same Step Functions machine with the shared portions of the state pre-populated.
