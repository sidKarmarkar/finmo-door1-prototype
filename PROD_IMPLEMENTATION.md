...
### Contacts, payment runs and chasing

**Contacts** are a counterparty master keyed per organisation, populated from ledger activity and from the accounting sync, with saved payout instruments held as tokenised references rather than raw account numbers. Entity resolution is the real work: the same vendor arrives spelled three ways across Xero, a bank statement and a manual bill, so matching runs deterministic first (external ID, exact name, account fingerprint) then embedding similarity for the tail, with anything ambiguous surfaced rather than merged silently. Merging two counterparties is reversible for the same reason. Payment behaviour is a rolling aggregate of settled invoices, days between due date and value date, which feeds both the collections policy and, later, the payment-timing model behind the forecast.

**Payment runs** are a batch saga over the existing single-payment saga: a run is a parent record with per-item children, each child idempotent on its own key so a partial failure never double-pays. Funding evaluation happens twice, at build time for the UI and again inside the transaction at execution, since balances move between the two. The surplus rule (a currency can only fund another currency with what it does not already owe within the same run) is a solver constraint, and at larger scale becomes a small optimisation across balances rather than a single-source pick. Approval attaches to the run total, and the audit record keeps both the batch decision and the individual outcomes, which is what an auditor asks for. Per-item failures are terminal states with reasons, never silent drops.

**Chasing** is a scheduled workflow per invoice: a sequence definition (offsets, tone, channel) instantiated on arming, with each step a scheduled job that re-checks invoice state before sending, so a paid invoice cancels its own remaining steps. Sends go through the existing notification service with suppression rules (no reminders to a customer already in dispute, rate limits per contact, quiet hours in their timezone) because the fastest way to lose a customer is an automated dunning email that should not have gone. The AI drafts copy from invoice facts and contact history; a human approves the sequence, not each message, and every send is logged against the invoice. The natural extension is choosing offsets per contact from their own payment behaviour rather than a fixed ladder.
...
### Insights and the cash-tools seam

The prototype's business account is the regulated, money-moving product. Cash tools is the separate unregulated data layer, external bank feeds and ERP data aggregated into a cash-flow model, and the Insights surface is where that layer meets the account. In production the insight engine is a batch job over the aggregated ledger: rule-plus-model detectors that each emit a typed insight with a quantified impact and a deep link to the account action that captures it. Wire-versus-local compares the customer's actual cross-border payments against available local rails and their fee schedule. FX-savings compares realised external conversions against a historic mid-market series (a provider such as currencylayer) net of Finmo's own margin, so the delta is sourced, not asserted. Liquidity and hedging detectors read the same forecast the runway uses. The AI role is classification and the natural-language rationale; the arithmetic is deterministic, because a savings claim a customer can audit is worth more than a clever one they cannot. The conversion metric, did an insight actually drive account usage, is the KPI the whole layer is built to move.

### Forecasting

Forecasting is a projection service over the aggregated cash-flow model, and its accuracy climbs a data ladder: detected recurring patterns first, then user-supplied end-dates and scenarios, then AR/AP and contract dates from the ERP, then enough history for seasonal decomposition. Recurring drivers are first-class records with a cadence and an optional end-date, which is the low-friction answer to the contract-expiry problem that treasury systems solve by demanding uploaded documents. Scenarios are non-destructive overlays on the driver set, so a what-if never mutates the base forecast. Confidence is deliberately not a single number until there is data to justify one: below a history threshold the service returns a qualitative band and an explicit "insufficient data" flag rather than a fabricated percentage, and statistical confidence (as in the VaR engine, where the distribution is known) is kept architecturally distinct from forecast confidence (which is data-limited). The model can graduate from statistical baselines to a learned payment-timing model per counterparty as history accumulates, with backtested accuracy shown to the customer.

### Dashboard analytics

**Cash runway** is a projection service rather than a stored report: starting position from the ledger, then unpaid receivables and scheduled payables bucketed by due date over the window, recomputed on demand and cached per tenant per day. It runs on the same read models as the rest of the analytics, so it costs a query, not a pipeline. The conservative rule that overdue receivables are excluded from the projected line is a business rule in the service, versioned like any other, because the moment a customer relies on the number the assumption behind it has to be inspectable. Aging buckets fall out of the same query with a reconciliation assertion in CI that the buckets sum to the ledger totals. The natural next step, once there is history, is replacing due-date arithmetic with a learned payment-timing model per counterparty, which is exactly where the AI earns its place: not in drawing the chart but in predicting *when* a given customer actually pays, against their own track record, with the backtested accuracy shown to the user.
...
Modelled as two more currencies in the ledger, not a parallel system, so the journal, conversion, risk and approval paths need no special cases. What is genuinely new sits at the edges: a custody and wallet provider (Fireblocks or Anchorage class) behind a `WalletProvider` interface, one deposit address per customer per coin per chain, and a chain listener that credits the ledger only after the confirmation threshold for that network. Deposits are idempotent on transaction hash, because a reorg replaying a credit is the failure mode that matters. Conversion between a stablecoin and fiat is an internal book transfer plus a liquidity position we manage, priced at a tighter margin for stable-to-stable than for stable-to-fiat, which reflects that one is a swap and the other is real FX. Travel Rule screening and on-chain analytics (Chainalysis class) run on deposits and withdrawals as another provider behind the screening interface. The regulatory point to be explicit about: holding stablecoins for customers is a distinct permission from e-money in most of the licence map, so this ships market by market as a configuration flag, not everywhere at once.

### Multiple entities

The data model is an organisation with many legal entities arranged as a tree (each entity carries a parent, with cycle prevention and a depth cap enforced server side), and tenancy moves from entity to organisation with an entity ID on every ledger row, account, document and policy. Roll-up balances are a recursive read over the subtree, cached per node and invalidated on posting, so a parent showing the group position stays cheap as the tree grows.

Onboarding a new entity branches on beneficial ownership. Where the declared owners match the parent's verified set, the KYB case reuses those verified person records and their existing screening results, and only the registry check on the new company runs, which is what makes same-day activation defensible rather than a shortcut. Where the owners differ, the full people step runs: identity, screening, and control person, with the group's risk rating carried over as a prior rather than a pass. The declaration itself is recorded and re-attested periodically, because ownership drifts and a stale "same owners" answer is a compliance gap. Both branches are the same Step Functions machine with different required states. Each entity carries its own KYB case, its own accounts and its own books, while the organisation carries the verified owners, control person and risk rating that a new entity inherits, which is exactly why the second application is short. Adding an entity is the same Step Functions machine with the shared portions of the state pre-populated.

Consolidation is a read model across entities, deliberately read only, with all write paths scoped to a single entity so nothing can post across two sets of books by accident. Intercompany transfers are two journal entries plus an FX leg, settled internally with no rail, and they need the accounting treatment written down up front: an intercompany payable and receivable pair, which is what makes the export to the customer's accounting system correct. Authorisation becomes a pair, the role and the entity scope, evaluated server side on every request.

### Accounting integrations

Xero and QuickBooks behind one `AccountingProvider` interface, OAuth per tenant with refresh tokens in Secrets Manager, and their webhooks driving incremental sync rather than polling where available. Pull bills and invoices into staging, map contacts to counterparties, then reconcile: match on external ID first, then on amount, date and name similarity through the same embedding path the payin matcher already uses. Write-back posts the payment against the source bill with our transaction ID attached, and is the leg that has to be transactional in the face of partial failure, so it runs as an outbox with retries and a visible failure state rather than a silent divergence between two systems of record. Sync conflicts (a bill edited on both sides) surface for a human rather than resolving by last-write-wins, because quietly overwriting someone's books is unrecoverable.

The AI role here is matching and classification, not sync. And the strategic payoff is data: due dates from the accounting system are what turn the forecast from a chart into something a customer will act on.
